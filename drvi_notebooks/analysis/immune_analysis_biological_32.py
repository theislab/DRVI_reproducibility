...
# %%
output_dir = proj_dir / 'plots' / 'immune_analysis_bio'
...
RUNS_TO_LOAD = get_runs_by_model_tags(
    ...
    ["DRVI_runs_adata_hvg_drvi_4.3", "DRVI_runs__DRVI_5.0", "DRVI_runs__DRVI_baselines_2.0"],
    {
        ...
        'DRVI2': 'Immune_comparison_analysis_32__DRVI2',
...
for method_name, embed in embeds.items():
    ...
    plt.title(f"{method_name} | DC1 vs {best_dim}")
...
present_ifn_markers = [g for g in HALLMARK_INTERFERON_ALPHA_RESPONSE if g in adata_full.var.index]
len(HALLMARK_INTERFERON_ALPHA_RESPONSE), len(present_ifn_markers)
...
for method_name, embed in embeds.items():
    ...
    # --- Scatterplot of IFN and best_dim ---
    plt.figure(figsize=(4, 4))
    ...
    plt.title(f"{method_name} | IFN vs {best_dim}")
    plt.tight_layout() # Ensures the legend isn't cut off when saving/displaying
    plt.show()
