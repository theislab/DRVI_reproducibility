...
for _, col in enumerate(_plot_columns):
    ...
    plt.subplots_adjust(left=0.1,
                        ...
                        hspace=0.1)   
    plt.tight_layout()
    dir_name = proj_dir / 'plots' / 'developmental_pancreas_analysis_v2'
...
# # DRVI interpretability pipeline
...
drvi.utils.pl.plot_latent_dims_in_heatmap(embed, "clusters_fine", title_col="title")
...
for dim_title, gene_scores in drvi.utils.tl.iterate_on_top_differential_vars(
    ...
    # isec = set(gene_scores[:20].index).intersection(["Cdkn1a", "Ppp1r14a", "Btbd17", "Tbc1d9", "Kif26a", "Megf11", "Epb42", "Scube1", "Shf", "Fam107b", "Selenom", "Smarcd2"])
    # isec = set(gene_scores[:20].index).intersection(["Hhex", "Sst", "Mef2c", "Dscam", "Ptprz1", "Spock3"])
    # isec = set(gene_scores[:20].index).intersection(["Irs4", "Ghrl", "Mboat4", "Lrrtm3"])
    # isec = set(gene_scores[:50].index).intersection(["Irx1", "Irx2"])
    isec = set(gene_scores[:20].index).intersection(["Arx", "Mafb"])
    # isec = set(gene_scores[:20].index).intersection(["Lrttm3", "Ctnna3", "Plcb4"])
    if len(isec) > 0:
        print(dim_title)
        print(len(isec))
        print(set(gene_scores[:20].index))
