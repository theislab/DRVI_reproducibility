...
adata.obs['CXCR3-binding active'] = adata.obs['CXCR3-binding'] > 2.5
...
# %%
cont_col = 'CXCR3-binding'
disc_col = 'CXCR3-binding active'

most_relevant_dim = {}
for method_name, embed in embeds.items():
    print(method_name)
    embed.obs[cont_col] = adata.obs[cont_col]
    embed.obs[disc_col] = adata.obs[disc_col]
    bench = DiscreteDisentanglementBenchmark(
        embed.X, discrete_target=embed.obs[disc_col], one_hot_target=None,
        dim_titles=embed.var['title']
    )
    bench.evaluate()
    scores = bench.get_results_details()['SMI'][True].sort_values(ascending=False)
    best_dim = scores.index[0]
    most_relevant_dim[method_name] = best_dim
    ...
    # --- Scatterplot of DC1 and best_dim ---
    ...
    ax.set_ylabel(f"{disc_col} score")
    
    # Move legend outside the plot
    sns.move_legend(ax, "upper left", bbox_to_anchor=(1.05, 1))
    
    # Fix the legend dots so they are visible
    ...
    plt.title(f"{method_name} | {disc_col} vs {best_dim}")
...
adata.obs['Responsing to metal'] = adata.obs['Response to metal'] > 1.5
adata.obs['Responsing to metal'] = adata.obs['Responsing to metal'].astype('category')

# %%
adata.obsm['X_umap_tmp'] = embeds['DRVI'][adata.obs.index].obsm['X_umap']
sc.pl.embedding(adata, "X_umap_tmp", color=['Responsing to metal', 'Response to metal'] + canonical_markers, ncols=3)

# %%

# %%
cont_col = 'Response to metal'
disc_col = 'Responsing to metal'
...
# %%
bench.get_results_details()['SMI'][True].sort_values(ascending=False)
...
# %%
net = pd.DataFrame({
    'source': ['Hypoxia'] * len(hypoxia_response_genes),
    'target': hypoxia_response_genes,
    'weight': [1.0] * len(hypoxia_response_genes)  # All genes contribute equally and positively
})

# 2. Run the scoring algorithm
# We'll use ULM (Univariate Linear Model), which is fast and robust for scRNA-seq
dc.mt.ulm(
    data=adata, 
    net=net,
    verbose=True
...
# decoupler stores the results in adata.obsm. ULM estimates are in 'ulm_estimate'
# Let's move the score into adata.obs so we can plot it easily
adata.obs['Hypoxia'] = adata.obsm['score_ulm']['Hypoxia']

# %%
adata.obs['Hypoxia active'] = adata.obs['Hypoxia'] > 6
adata.obs['Hypoxia active'] = adata.obs['Hypoxia active'].astype('category')
sc.pl.embedding(adata, "X_umap_tmp", color=['Hypoxia active', 'Hypoxia'])
