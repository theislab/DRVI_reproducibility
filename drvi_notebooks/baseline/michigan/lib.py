...
def log(x, opt):
	...
	"""
	return tf.math.log(x + opt.epsilon_use)
...
def compute_gp(x, x_gen, disc, opt):
	...
	"""
	epsilon_x = tf.compat.v1.random_uniform([], 0.0, 1.0)
	x_hat = x * epsilon_x + (1 - epsilon_x) * x_gen
	d_hat, _ = disc(x_hat, opt, reuse = True)
	gradients = tf.compat.v1.gradients(d_hat, x_hat)[0]
...
def MutualInformationLowerBound(c_rec, c_sample, opt):
	...
	ll_con_dist = tfd.Normal(est_vec, std_vec)
...
def c_mutual_mu_var_entropy(z_gen_mean_v, z_gen_std_v, c_rec_sample, opt):
	...
	con_gen = -0.5 * (z_norm * z_norm + tf.math.log(z_var) + np.log(2 * np.pi))
...
def estimate_minibatch_mss_entropy(z_gen_mean_v, z_gen_std_v, c_rec_sample, opt):
	...
	part_4 = - tf.concat([output, outpart_3], axis = 1)/tf.cast(dataset_size, tf.float32)

	part_1 = tf.ones((tf.shape(c_rec_sample)[0], tf.shape(c_rec_sample)[0]))/tf.cast(tf.shape(c_rec_sample)[0] - 1, tf.float32)
	part_2 = tf.ones((tf.shape(c_rec_sample)[0], tf.shape(c_rec_sample)[0]))
	part_2 = - tf.linalg.band_part(part_2, 1, 0)/tf.cast(dataset_size, tf.float32)
	part_3 = tf.eye(tf.shape(c_rec_sample)[0]) * (2/tf.cast(dataset_size, tf.float32) - 1/tf.cast(tf.shape(c_rec_sample)[0] - 1, tf.float32))
...
def estimate_entropy(z_gen_mean_v, z_gen_std_v, c_rec_sample, opt):
	...
	"""
	weights = - log(tf.cast(tf.shape(z_gen_mean_v)[0], tf.float32), opt)
