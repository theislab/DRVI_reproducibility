...
class Adam_Prediction_Optimizer(optimizer.Optimizer):
    ...
    def _create_slots(self, var_list):
        ...
        if create_new:
            dtype = first_var.dtype.base_dtype
            # Isolate beta accumulators per optimizer instance (multiple Adams share global var store).
            with tf.compat.v1.variable_scope(self._name):
                with tf.compat.v1.colocate_with(first_var):
                    self._beta1_power = tf.compat.v1.get_variable(
                        "beta1_power",
                        shape=[],
                        initializer=tf.compat.v1.constant_initializer(self._beta1),
                        trainable=False,
                        dtype=dtype,
                    )
                    self._beta2_power = tf.compat.v1.get_variable(
                        "beta2_power",
                        shape=[],
                        initializer=tf.compat.v1.constant_initializer(self._beta2),
                        trainable=False,
                        dtype=dtype,
                    )
